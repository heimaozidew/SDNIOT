...
from mininet.nodes import ( Node,Sensor,Station, Host, OVSKernelSwitch, DefaultController,AP,APSensor,ZigBee,ZigSensor,
                           Controller )
from mininet.nodelib import NAT
from mininet.links import Link, Intf
...
class Mininet( object ):
    ...
    def __init__( self, topo=None, switch=OVSKernelSwitch, host=Host,sensor=Sensor,station=Station,
                  controller=DefaultController, link=Link, intf=Intf,ap=AP,apsensor=APSensor,
    ...
    def addZigBee( self, name, cls=None, **params ):
        """Add sensor.
           name: name of host to add
           cls: custom host class/constructor (optional)
           params: parameters for host
    ...
    def delSwitch( self, host ):
        "Delete a host"
        self.delNode( host, nodes=self.switches )
    ...
    def addAP( self, name, cls=None, **params ):
        ...
        self.nameToNode[ name ] = sw
        return sw
    def addZigSensor( self, name, cls=None, **params ):
        """Add switch.
           name: name of switch to add
           cls: custom switch class/constructor (optional)
           returns: added switch
    ...
    def set_network_type(self, node ,port, tag):
        self.network_type[port.split("-")[0]]=tag
        node.cmd("ovs-vsctl set port {} tag={}".format(port,tag))

    def set_allow_network_type(self,node,port,trunk):
        node.cmd('ovs-vsctl set Port {} trunk={}'.format(port,trunk))
    ...
    def addNAT( self, name='nat0', connect=True, inNamespace=False,linktype='eth',
                **params):
        ...
        if connect:
            ...
            self.addLink( nat, connect, linktype='eth')
    ...
    def addLink( self, node1, node2,linktype='eth', port1=None, port2=None,
                 cls=None, **params ):
        ...
        link = cls( node1, node2, linktype=linktype,**options )
        self.links.append( link )
        if node1 in self.hosts and node2 in self.switches:
            if node2 not in self.nodeLswitch:
                self.nodeLswitch[node2]=[]
            self.nodeLswitch[node2].append(node1)
        elif node2 in self.hosts and node1 in self.switches:
            if node1 not in self.nodeLswitch:
                self.nodeLswitch[node1]=[]
            self.nodeLswitch[node1].append(node2)
        else:
            pass
